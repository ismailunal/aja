defmodule A do
  ...
  ## Examples
      ...
      iex> ~i"abc#{['def' | "ghi"]}"
      ["abc", ['def' | "ghi"]]
      iex> ~i"Joseph Joestar"
      "Joseph Joestar"

  String interpolation uses `A.IO.to_iodata/1` instead of `to_string/1`, which will preserve lists.

  IO-data can be used by most I/O operations, like from the `IO` module, anything using a socket
  or Phoenix templates. To see the equivalent string, you can use `IO.iodata_to_binary/1`:
